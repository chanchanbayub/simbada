    ...
    <div class="card-body">
        <form id="form_penderekan" autocomplete="off">
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="ukpd_id" name="ukpd_id" style="width:100% ;">
                    <option value="">--Silahkan Pilih--</option>
                    <?php foreach ($ukpd as $ukpd) : ?>
                        <?php if ($ukpd["id"] == session('ukpd_id')) : ?>
                ...
                <div class="invalid-feedback" id="error_ukpd_id">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="invalid-feedback" id="error_unit_id">
                </div>
            </div>

            <div class="form-group">
                <label for="bap_id">Nomor Bap :</label>
                <select class="form-control" id="bap_id" name="bap_id" style="width:100% ;" disabled>
                    <option value="">--Silahkan Pilih--</option>
                </select>
                <div class="invalid-feedback" id="error_bap_id">
                </div>
            </div>
    ...
    $("#unit_id").change(function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                let bap_id = "<option value=''> --Silahkan Pilih-- </option>";
                if (response.bap.length > 0) {
                    $("#bap_id").removeAttr('disabled', 'disabled');
                    response.bap.forEach(function(e) {
                        bap_id += `<option value='${e.id}'> ${e.noBap} </option>`
                    });
                } else {
                    $("#bap_id").attr('disabled', 'disabled');
                    bap_id += "<option value=''> --Silahkan Pilih-- </option>";
                }
                $('#bap_id').html(bap_id);
            }
        });
            ...
            success: function(response) {
                ...
                if (response.error) {
                    if (response.error.unit_id) {
                        ...
                        $("#error_unit_id").html(response.error.unit_id);
                    } else {
                    ...
                    if (response.error.bap_id) {
                        ...
                        $("#error_bap_id").html(response.error.bap_id);
                    } else {
